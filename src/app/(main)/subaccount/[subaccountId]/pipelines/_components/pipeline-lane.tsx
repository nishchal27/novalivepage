...
const PipelineLane: React.FC<PipelaneLaneProps> = ({
  ...
  //create laneAmt every time tickets changes
  //memoization
  ...
  return (
    <Draggable
      ...
      key={laneDetails.id}
    >
      {/* Issue: react-beautiful-dnd absolute container is using fixed by default,
      due this when we drag elements it snaps to some other location on the page */}
      {/* so this is solution  */}
      {(provided, snapshot) => {
        if (snapshot.isDragging) {
          //@ts-ignore
          // if it's not working, tweak this number until it looks perfect for all screens
