...
const UserDetails = ({ id, type, subAccounts, userData }: Props) => {
  ...
  const onChangePermission = async (
    ...
    // Update user permissions via API call
    const response = await changeUserPermissions(
      // Determine permissions ID or generate a new one if not provided
    ...
    if (type === "agency") {
      // Save activity logs notification if user type is agency
    ...
    if (response) {
      ...
      // Update local permissions state if available
    ...
    // Refresh router and reset loading state
    router.refresh();
